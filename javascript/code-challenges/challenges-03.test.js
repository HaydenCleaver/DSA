...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter(ele => forbiddenValues.indexOf(ele) === -1);
};
...
const getStatName = (arr, minBaseStat) => {
  ...
  arr.filter(ele => ele.baseStat > minBaseStat ? newArr.push(ele.stat.name) : false);
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter(ele => ele.children ? false : ele);
};
...
const evenOddNumericValues = (arr) => {
  return arr.filter(ele => typeof ele === 'number').map(val => val % 2 === 0 ? 'even' : 'odd');
};
